...
def dwnpage(a, b):
    ...
    #return downloadPage(a.encode('utf-8'), b, headers=h) if sys.version_info[0] == 3 else downloadPage(a, b, headers=h)
...
class CSFDLite(Screen):
    ...
    def fetchFailed(self, string):
        print("[CSFDLite] fetch failed %s"%string)
        self["statusbar"].setText(toStr("Stále stahuji z CSFD: " + string))
    ...
    def showDetails(self):
        ...
        if self.resultlist and self.Page == 0:
            ...
            dwnpage(fetchurl,localfile).addCallback(self.CSFDquery2).addErrback(self.fetchFailed)
            self["menu"].hide()
            self.resetLabels()
    ...
    def getCSFD(self):
        def removeShit(val):
            try:
                val = val.lower()
                val = re.sub("(\[b\].*?\[\/b\])", "", val, re.IGNORECASE | re.DOTALL)
                val = re.sub("(\[i\].*?\[\/i\])", "", val, re.IGNORECASE | re.DOTALL)
                val = re.sub("(\[light\].*?\[\/light\])", "", val, re.IGNORECASE | re.DOTALL)
        ...
        if self.eventName != "":
            ...
            self.eventName = removeShit(self.eventName).replace('[i]','').replace('[I]','').replace('[b]','').replace('[B]','').replace('[/i]','').replace('[/I]','').replace('[/b]','').replace('[/B]','').replace('[light]','').replace('[/light]','').replace('[LIGHT]','').replace('[/LIGHT]','')
    ...
    def CSFDquery(self, string):
        ...
        if '<h1 itemprop="name">' in self.inhtml:
            ...
            for odkaz, filmnazev, filminfo, filmGenre in self.hledejVse('<h3.*?<a href="/film/(.*?)".*?"film-title-name">(.*?)</a>(.*?)</h3>.*?<span.*?class="info">(.*?)</span>', seznamcely):
            ...
            # toto uz fakt netusim co sa to tu vola dokola jak u kokotov
            # -----------------------------------------------------------------------------------------
            # self["statusbar"].setText(toStr("Hledání v CSFD pro: %s" % (self.nazeveventuproskin)))
            ...
            # self.puvodniurl = fetchurl
            # print("[CSFDLite] Downloading Query " + fetchurl + " to " + localfile)
            # dwnpage(fetchurl,localfile).addCallback(self.CSFDquery_dotaz2).addErrback(self.fetchFailed)

    def CSFDquery_dotaz2(self, string):			
        print("[CSFDquery_dotaz2] %s"%string)
        self["statusbar"].setText(toStr("Stahování z CSFD dokončeno pro %s" % (self.nazeveventuproskin)))
        ...
        if '<h1 itemprop="name">' in self.inhtml:
            ...
            if self.jmeno2 != "" and self.jmeno2 != " ":
                ...
                self.puvodniurl = fetchurl
                print("[CSFDLite] Downloading Query " + fetchurl + " to " + localfile)
                dwnpage(fetchurl,localfile).addCallback(self.CSFDquery_dotaz3).addErrback(self.fetchFailed)
            else:
                self.projitSeznam()
        else:
    ...
    def projitSeznam(self):	
        ...
        try:
            ...
            for nazevinfo, odkaz, nazevfilmu, rok in self.resultlist:
                if norm(nazevfilmu).lower() == norm(unquote(self.eventName)):
                    shoda += [(nazevinfo, odkaz, rok)]
                elif norm(unquote(self.eventName)).lower().startswith(norm(nazevfilmu)):
                    shoda += [(nazevinfo, odkaz, rok)]
            if len(shoda) == 1:
        ...
        if 'class="film-info"' in self.inhtml:
            ...
            if not 'class="empty-image"' in self.inhtml:
                ...
                if posterurl != "":
                    ...
                    try:
                        dwnpage(posterurl,localfile).addCallback(self.CSFDPoster).addErrback(self.fetchFailed)
                    except:
                        print("no jpg poster!")
            ...
            if druhastranka:
                ...
                print("[CSFDLite] downloading query " + fetchurl + " to " + localfile)
                dwnpage(fetchurl,localfile).addCallback(self.CSFDquery3).addErrback(self.fetchFailed)
            else:
    ...
    def kontrolaUpdate(self):
        ...
        if kontrolovatupdate:
            ...
            dwnpage('https://raw.githubusercontent.com/skyjet18/enigma2-plugin-extensions-csfdlite/master/version.txt', self.cisloverze).addCallback(self.porovnaniVerze).addErrback(self.fetchFailed)
    ...
    def provedeniUpdate(self, odpoved):
        if odpoved:
            if self.kontejnerfunguje:
                dwnpage('https://github.com/skyjet18/enigma2-plugin-extensions-csfdlite/blob/master/releases/' + self.koncovkasouboru+'?raw=true', '/tmp/CSFDLite.tar.gz').addCallback(self.rozbaleniTaru).addErrback(self.fetchFailed)
            else:
